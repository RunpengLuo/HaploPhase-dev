...
def get_assignment_rand():
    return random.choices(population=[0,1], weights=[0.5, 0.5], k=1)[0]
...
def gfa_to_fasta(gfa_file, fasta_file):
    os.system(f"""awk '/^S/{{print ">"$2"\\n"$3}}' {gfa_file} > {fasta_file}""")
    return
